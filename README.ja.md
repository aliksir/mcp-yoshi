...
| IN-014 | レスポンス内認証情報 | stdout/stderr内の認証情報残留検出（AWS/OpenAI/GitHub等のキー、Bearer Token、秘密鍵） |
| IN-015 | パラメータオーバーライド | `overrideConfig` キー + `mcpServerConfig`/`NODE_OPTIONS`/`executablePath` 同居の Allowlist Bypass 攻撃（CVE-2026-40933 関連） |
| IN-017 | パストラバーサル | `../` ディレクトリ移動 + `basePath`/`filePath`/`filename` への機密パス指定（`/etc/`, `/root/`, `C:\Windows\`, `/proc/`） |
| IN-018 | クエリインジェクション | SQL/Cypher/NoSQL injection — BLOCK: `UNION SELECT`, `DROP TABLE`, `MATCH...DELETE`, `;--` / WARN: `' OR '`, `sleep()` |
| IN-019 | サンドボックス脱出 | vm/Function/global access（`globalThis.process.mainModule.require`, `constructor.constructor()` vm2 escape 等） |
| IN-020 | ヘッダ偽装 | 信頼境界バイパス（`x-request-from: internal`, `x-forwarded-for: 127.0.0.1` 等） |
| IN-021 | ブラウザ起動RCE | Puppeteer/Playwright `executablePath` にシェルバイナリ指定（`/bin/sh`, `/usr/bin/nc` 等） |
