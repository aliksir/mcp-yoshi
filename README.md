...
初回呼び出し時にツール定義のハッシュを記録し、以降の呼び出しで変更があればWARNを発生させます。
...
```json
{
  ...
  "severity": {
    "WARN": ["highEntropy", "pii", "suspiciousUrls", "base64Payload"],
    "BLOCK": ["apiKeys", "privateKeys", "promptInjection", "shellCommands", "scriptInjection", "toolTampering", "envValues", "asciiSmuggling"]
  }
}
```
...
| `severity.WARN` | `["highEntropy", "pii", "suspiciousUrls", "base64Payload"]` | WARN判定するチェックID |
...
- **Rug Pull検出**: ツール定義ハッシュはプロセスのインメモリに保持されるため、Claude Codeセッションを跨いだ検出はできません
